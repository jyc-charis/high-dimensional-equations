...
class SolveAllenCahn ( object ):
    ...
    def __init__ (self , sess ):
        ...
        self.n_time = 20
        self.n_layer = 4
        self.n_neuron = [ self.d, self.d, self.d, self.d]
        self.batch_size = 64
        self.valid_size = 256
        self.n_maxstep = 2000
    ...
    def train ( self ):
        ...
        # train operations
        self.global_step = \
            tf.compat.v1.get_variable ('global_step', [],
        ...
        for _ in range ( self . n_maxstep +1):
            ...
            self.sess. run ( self . train_op , feed_dict ={ self.dW: dW_train ,
                                                              self .X: X_train ,
    ...
    def build ( self ):
        ...
        self .dW = tf.compat.v1.placeholder (tf. float64 ,
                                    [None , self.d, self. n_time ],name ='dW')
        self .X = tf.compat.v1.placeholder (tf. float64 ,[None , self .d, self . n_time +1] ,name ='X')
        self . is_training = tf.compat.v1.placeholder (tf. bool )
        self .Y0 = tf. Variable (tf.random.uniform ([1] ,
                                                     minval = self . Yini [0] ,
                                                     maxval = self . Yini [1] ,
                                                     dtype =tf. float64 ));
        self .Z0 = tf. Variable (tf.random.uniform ([1 , self .d],minval =-.1,maxval =.1 ,dtype =tf. float64 ))
        self . allones = \
            tf. ones ( shape =tf.stack ([ tf. shape ( self .dW )[0] , 1]) ,dtype =tf. float64 )
        ...
        with tf.compat.v1.variable_scope ('forward'):
            for t in range (0, self .n_time -1):
                Y = Y - self . f_tf ( self . t_stamp [t],
                                     self .X[:, :, t], Y, Z)* self .h
                Y = Y + tf.compat.v1.reduce_sum (Z* self .dW [:, :, t], 1,keep_dims = True )
                Z = self . _one_time_net ( self .X[:, :, t+1] ,str (t +1))/ self .d
            # terminal time
            Y = Y - self . f_tf ( self . t_stamp [ self .n_time -1] ,
                                 self .X[:, :, self .n_time -1] ,
                                 Y, Z)* self .h
            Y = Y + tf.compat.v1.reduce_sum (Z* self .dW [:, :, self .n_time -1] , 1,
                                    keep_dims = True )
    ...
    def g_tf (self , t, X):
        # terminal conditions
        return 0.5/(1 + 0.2* tf.compat.v1.reduce_sum (X**2 , 1, keep_dims = True ))
    ...
    def _one_layer (self , input_ , out_sz ,
        ...
        with tf.compat.v1.variable_scope ( name ):
            ...
            w = tf.compat.v1.get_variable ('Matrix',
    ...
    def _batch_norm (self , x, name ):
        ...
        with tf.compat.v1.variable_scope ( name ):
            ...
            gamma = tf.compat.v1.get_variable("gamma", params_shape, tf.float64,
                                        initializer = tf.random_normal_initializer(
                                        0.1, 0.5))
            mv_mean = tf.compat.v1.get_variable("moving_mean", params_shape, tf.float64,
                                        initializer = tf.constant_initializer(
                                        0.0), trainable = False)
            mv_var = tf.compat.v1.get_variable("moving_variance",  params_shape, tf.float64,
                                        initializer = tf.constant_initializer(
                                        1.0), trainable = False)
